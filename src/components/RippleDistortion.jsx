...
export default function RippleDistortion({
  ...
  useEffect(() => {
    ...
    gl.clearColor(0.07, 0.02, 0.03, 1);
    ...
    const waves = Array.from({ length: MAX_WAVES }, () => ({
      x: 0, y: 0, scale: START_SCALE, target: START_SCALE, size: 1, opacity: 0,
    }));
    let current = 0;
    ...
    const displacementTarget = new RenderTarget(gl, {
      width: 2, height: 2, depth: false, minFilter: gl.LINEAR, magFilter: gl.LINEAR,
  ...
  return (
    <div
      ref={mountRef}
      className={`ripple-distortion ${clear ? "is-clear" : "is-blurred"} ${className}`.trim()}
      style={style}
    />
  );
}
