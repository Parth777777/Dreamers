...
// Helper to auto-pause videos that are out of view
    ...
    >
        ...
        >
          ...
          >
            {slides.map((slide, index) => (
              ...
              >
                {slide.src.includes("youtube.com") || slide.src.includes("instagram.com") ? (
                  <iframe
                    src={slide.src}
                    className="h-full w-full select-none object-cover pointer-events-none border-0"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                  ></iframe>
                ) : slide.src.match(/\.(mp4|mov|webm)$/i) ? (
                  <VideoSlide src={slide.src} isActive={index === selected} />
                ) : (
                  <Image
                    src={slide.src}
                    alt={slide.alt}
      ...
      {showCaption && active?.title && (
        ...
        >
          ...
          {active.href && (
            <div className="mt-6">
              <Button href={active.href}>{active.linkLabel || "View Project"}</Button>
            </div>
          )}
        </div>
      )}
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          {slides.map((_, index) => (
            <button
              ...
              onClick={() => goTo(index)}
              className={cn(
                "size-2 rounded-full bg-paper transition-opacity",
